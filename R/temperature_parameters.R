...
temperature_parameters = function( p=list(), project_name="temperature", project_class="core", ...) {
  ...
  # simple run of carstm. There are two types:
    ...
    p = parameters_add_without_overwriting( p,
      ...
      areal_units_constraint_ntarget = 40,  # n time slices req in each au
      areal_units_constraint_nmin = 10,   # n time slices req in each au
      ...
      if ( !exists("carstm_model_formula", p)  ) {
        p$carstm_model_formula = as.formula( paste(
         p$variabletomodel, ' ~ 1',
          ...
          ' + f( space_time, model="besag", graph=slot(sppoly, "nb"), group=time_space, scale.model=TRUE, constr=TRUE, hyper=H$bym2, control.group=list(model="ar1", hyper=H$ar1_group) ) '
          ) )
      }
