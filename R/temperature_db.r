...
temperature_db = function ( p=NULL, DS, varnames=NULL, yr=NULL, ret="mean", dyear_index=NULL, redo=FALSE, ... ) {
  ...
  if (DS %in% c("misc","misc.redo")) {
    ...
    misc$lon[i] = floor(misc$lon[i]/100) + round((misc$lon[i]/100 - floor(misc$lon[i]/100 ))/60 * 100, 6)

    i = which( misc$lat > 90)  # deg-min
    misc$lat[i] = floor(misc$lat[i]/100) + round((misc$lat[i]/100 - floor(misc$lat[i]/100 ))/60 * 100, 6)
    ...
    for ( yt in yr ) {
      ...
      if (!is.null(profile)) {
        ...
        if (length( igood) > 0 ) {
          ...
          profile$id =  paste(
            floor(profile$plon/p$inputdata_spatial_discretization_planar_km + 1) * p$inputdata_spatial_discretization_planar_km,
            floor(profile$plat/p$inputdata_spatial_discretization_planar_km + 1) * p$inputdata_spatial_discretization_planar_km,
      ...
      locs$plon = floor(locs$plon / p$inputdata_spatial_discretization_planar_km + 1 ) * p$inputdata_spatial_discretization_planar_km
      locs$plat = floor(locs$plat / p$inputdata_spatial_discretization_planar_km + 1 ) * p$inputdata_spatial_discretization_planar_km
